	...
	public static void handleCollisions(){
		...
		for(Collider c : enemyHurtboxes){
			if(c.collides(playerHitbox)){
				c.onCollide(CollisionType.PLAYER_HITBOX_COLLISION, null);
				playerHitbox.onCollide(CollisionType.ENEMY_HURTBOX_COLLISION, null);
			}
		}
		//For every wall, see if it collides with the player hitbox
		for(Wall w : walls){
			if(w.collides(playerHitbox)){
				playerHitbox.onCollide(CollisionType.WALL_COLLISION, w.getWall());
				w.onCollide(CollisionType.PLAYER_HITBOX_COLLISION, null);
			}
		}
		...
		for(Collider c : playerHurtboxes){
			for(Collider ce : enemyHitboxes){
				if(c.collides(ce)){
					c.onCollide(CollisionType.ENEMY_HITBOX_COLLISION, null);
					ce.onCollide(CollisionType.PLAYER_HURTBOX_COLLISION, null);
				}
			}
		}
